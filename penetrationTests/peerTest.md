...
Through the penetration testing exercises, several critical lessons emerged about building secure and resilient web applications:

- Rate limiting and resource controls: Multiple attacks revealed unrestricted API usage, allowing repeated registrations, orders, or deletions without any cap. This highlighted the importance of implementing middleware such as express-rate-limit to prevent denial‑of‑service attempts and abuse of endpoints.
- Authentication and authorization enforcement: Tests showed that missing or weak authentication checks allowed unauthorized deletion of franchises and potential privilege escalation. Strong enforcement of token validation and role‑based access control is essential to protect sensitive operations.
- Input validation and uniqueness constraints: Duplicate account creation and improper input handling demonstrated the need for strict validation. Enforcing unique email constraints at both the database and application level prevents integrity failures and account duplication.
- Secure handling of secrets and credentials: Hardcoded admin credentials and exposed configuration files in CI/CD artifacts underscored the risks of poor secret management. Environment variables, secret injection at runtime, and proper .gitignore practices are necessary to avoid credential leaks.
- Trusting the server as the source of truth: The ability to manipulate pizza order prices by altering request payloads showed why client‑supplied values must never be trusted. The backend must always calculate sensitive fields (like prices) using authoritative database values.
- Access control for sensitive data: Some endpoints returned franchise information without authentication, which could be exploited for social engineering. APIs should carefully distinguish between public and private data, exposing only what is necessary.
- Default configurations and credential hygiene: Using default admin accounts or leaving initial passwords unchanged creates easy attack vectors. Secure deployment requires removing defaults, enforcing password resets, and rotating credentials regularly.
- CI/CD pipeline security: Publicly accessible build artifacts containing secrets highlighted the importance of artifact hygiene. Sensitive files must be excluded from uploads, and secrets should only be injected securely during deployment.

**Overall Takeaway**
The combined testing showed that while functional code may work correctly, security requires deliberate design choices. Protecting APIs involves more than preventing obvious errors — it requires layered defenses: rate limiting, strict validation, strong authentication, careful secret management, and secure CI/CD practices. The overarching lesson is that security must be integrated into every stage of development, from schema design to deployment pipelines, rather than treated as an afterthought.
